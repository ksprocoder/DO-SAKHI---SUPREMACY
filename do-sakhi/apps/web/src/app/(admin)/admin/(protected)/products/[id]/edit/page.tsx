  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setTitle(p.title || "");
        setSlug(p.slug || "");
        if (p.slug) setSlugManual(true);
        setShortDesc(p.short_description || "");
        setDescription(p.description || "");
        setRibbon(p.ribbon || "");
        setProductType(p.product_type || "suit_set");
        setStatus(p.status || "draft");
        setFulfillment(p.fulfillment_type || "ready_to_ship");
        setCustomTailoring(p.custom_tailoring_available || false);
        setMadeToOrder(p.is_made_to_order || false);
        setFabricType(p.fabric_type || "");
        setFabricComp(p.fabric_composition || "");
        setFabricFeel(p.fabric_feel || "");
        setCare(p.care_instructions || "");
        setFitNote(p.fit_note || "");
        setSilhouette(p.silhouette || "");
        setNeckline(p.neckline || "");
        setSleeve(p.sleeve_type || "");
        setKurtiLen(p.kurti_length || "");
        setBottomType(p.bottom_type || "");
        setDupatta(p.dupatta_included || false);
        setPocket(p.pocket_available || false);
        setEmbroidery(p.embroidery_detail || "");
        setPrintDetail(p.print_detail || "");
        setLeadMin(p.lead_time_min_days ? String(p.lead_time_min_days) : "2");
        setLeadMax(p.lead_time_max_days ? String(p.lead_time_max_days) : "7");
        setSeoTitle(p.seo_title || "");
        setSeoDesc(p.seo_description || "");
        setCollectionSlugs(p.collectionSlugs ? p.collectionSlugs.join(", ") : "");
        ...
        if (p.variants && p.variants.length > 0) {
          setVariants(p.variants.map((v: any) => ({
            ...
            sizeNumeric: v.size_numeric || "",
            priceInr: v.price_inr ? String(v.price_inr) : "",
            compareAtPriceInr: v.compare_at_price_inr ? String(v.compare_at_price_inr) : "",
            costPriceInr: v.cost_price_inr ? String(v.cost_price_inr) : "",
            stockQuantity: v.stock_quantity ? String(v.stock_quantity) : "0",
            lowStockThreshold: v.low_stock_threshold ? String(v.low_stock_threshold) : "2",
          })));
        }
  ...
  return (
    <AdminShell>
      <div className="max-w-4xl mx-auto px-10 py-10">
        ...
        {success && (
          <div
            className="mb-8 p-4 text-xs uppercase tracking-widest text-center"
            style={{
              background: "rgba(167,111,77,0.1)",
          ...
          >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-6 py-3 text-xs uppercase tracking-[0.15em] transition-all disabled:opacity-50"
                style={{
                  background: "transparent",
              ...
              <button
                ...
                style={{
                  background: (saving || loading) ? "rgba(7,63,52,0.5)" : "#073F34",
